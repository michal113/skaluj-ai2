...
(function () {
  ...
  /* ---------- STYLE ---------- */
  const CSS = `
  .cb-launcher{position:fixed;right:24px;bottom:24px;z-index:99998;height:60px;width:60px;border:none;cursor:pointer;border-radius:50%;
    background:linear-gradient(135deg,#2f6fe0,#2456b0);color:#fff;box-shadow:0 14px 40px rgba(47,111,224,.42),0 4px 12px rgba(15,30,60,.18);
    display:grid;place-items:center;transition:transform .35s cubic-bezier(.22,1,.36,1);overflow:hidden}
  .cb-launcher:hover{transform:translateY(-2px) scale(1.03)}
  .cb-launcher .ic{width:26px;height:26px;transition:transform .4s cubic-bezier(.22,1,.36,1)}
  ...
  .cb-launcher .pulse{position:absolute;inset:0;border-radius:50%;animation:cbpulse 2.6s infinite}
  @keyframes cbpulse{0%{box-shadow:0 0 0 0 rgba(47,111,224,.45)}70%{box-shadow:0 0 0 16px rgba(47,111,224,0)}100%{box-shadow:0 0 0 0 rgba(47,111,224,0)}}
  .cb-badge{position:fixed;right:22px;bottom:66px;z-index:99999;background:#ff5a5f;color:#fff;font:600 11px "Geist Mono",monospace;
    min-width:20px;height:20px;border-radius:10px;display:grid;place-items:center;padding:0 6px;box-shadow:0 2px 8px rgba(0,0,0,.2);transition:opacity .3s}
  .cb-launcher.open ~ .cb-badge{opacity:0;pointer-events:none}
  .cb-panel{position:fixed;right:24px;bottom:96px;z-index:99999;width:390px;max-width:calc(100vw - 32px);height:600px;max-height:calc(100vh - 130px);
    background:#fff;border:1px solid rgba(0,0,0,.10);border-radius:20px;box-shadow:0 30px 80px rgba(15,30,60,.22),0 8px 24px rgba(15,30,60,.10);
    display:flex;flex-direction:column;overflow:hidden;font-family:"Geist",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#0b0d10;
  ...
  .cb-head{position:relative;padding:16px 18px;display:flex;align-items:center;gap:12px;color:#fff;background:linear-gradient(135deg,#2f6fe0,#2456b0);overflow:hidden}
  .cb-head::after{content:"";position:absolute;right:-40px;top:-60px;width:180px;height:180px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,.18),transparent 70%)}
  .cb-ava{position:relative;width:42px;height:42px;border-radius:12px;background:rgba(255,255,255,.16);display:grid;place-items:center;flex:none}
  .cb-ava svg{width:24px;height:24px}.cb-ava .dot{position:absolute;right:-2px;bottom:-2px;width:12px;height:12px;border-radius:50%;background:#31d158;border:2.5px solid #2f6fe0}
  .cb-hmeta{flex:1;min-width:0}.cb-hmeta b{display:block;font-size:15px;font-weight:600;letter-spacing:-.01em}
  .cb-hmeta span{font-size:11.5px;opacity:.85;display:flex;align-items:center;gap:5px}.cb-hmeta span i{width:6px;height:6px;border-radius:50%;background:#31d158;display:inline-block}
  .cb-hbtn{background:rgba(255,255,255,.12);border:none;color:#fff;width:32px;height:32px;border-radius:9px;cursor:pointer;display:grid;place-items:center;transition:background .2s}
  .cb-hbtn:hover{background:rgba(255,255,255,.24)}
  ...
  .cb-msg .mava{width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#2f6fe0,#2456b0);display:grid;place-items:center;flex:none}
  .cb-msg .mava svg{width:16px;height:16px;color:#fff}
  ...
  .cb-msg.user{flex-direction:row-reverse}.cb-msg.user .cb-bubble{background:linear-gradient(135deg,#2f6fe0,#2456b0);color:#fff;border-bottom-right-radius:5px}
  ...
  .cb-chip:hover{border-color:#2f6fe0;color:#2f6fe0;background:rgba(47,111,224,.10)}
  .cb-book{margin:2px 0 4px 37px;max-width:82%;border:1px solid rgba(0,0,0,.10);border-radius:14px;overflow:hidden;background:#fff;animation:cbin .4s cubic-bezier(.22,1,.36,1) both}
  .cb-book-top{padding:13px 15px;background:rgba(47,111,224,.10);display:flex;align-items:center;gap:10px}
  .cb-book-top svg{width:20px;height:20px;color:#2f6fe0;flex:none}.cb-book-top b{font-size:13.5px}.cb-book-top span{display:block;font-size:11.5px;color:#565d68;margin-top:1px}
  .cb-book .cta{display:block;margin:12px 15px 14px;text-align:center;background:linear-gradient(135deg,#2f6fe0,#2456b0);color:#fff;text-decoration:none;font-size:13.5px;font-weight:500;padding:11px;border-radius:10px;transition:opacity .2s}
  .cb-book .cta:hover{opacity:.92}.cb-book iframe{width:100%;height:520px;border:0;display:block}
  .cb-foot{padding:12px 14px;border-top:1px solid rgba(0,0,0,.10);background:#fff}
  .cb-inrow{display:flex;align-items:flex-end;gap:9px;background:#f7f8fa;border:1px solid rgba(0,0,0,.10);border-radius:14px;padding:6px 6px 6px 14px;transition:border-color .2s}
  ...
  .cb-send{width:38px;height:38px;flex:none;border:none;border-radius:11px;cursor:pointer;background:linear-gradient(135deg,#2f6fe0,#2456b0);color:#fff;display:grid;place-items:center;transition:transform .2s,opacity .2s}
  .cb-send:hover{transform:scale(1.05)}.cb-send:disabled{opacity:.4;cursor:default;transform:none}
  .cb-cred{text-align:center;font:10.5px "Geist Mono",monospace;color:#8b929c;margin-top:8px;letter-spacing:.04em}.cb-cred b{color:#2f6fe0;font-weight:500}
  @media(max-width:480px){.cb-panel{right:12px;left:12px;bottom:88px;width:auto;height:calc(100vh - 110px)}.cb-launcher{right:16px;bottom:16px}}
  `;
  ...
  wrap.innerHTML = `
    ...
    <div class="cb-panel" id="cbPanel" role="dialog" aria-label="Czat skaluj.ai">
      <div class="cb-head">
        <div class="cb-ava" style="color:#fff">${AV}<span class="dot"></span></div>
        <div class="cb-hmeta"><b>Asystent skaluj.ai</b><span><i></i> Online — odpowiada od razu</span></div>
        <button class="cb-hbtn" id="cbClose" aria-label="Zamknij"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
      </div>
